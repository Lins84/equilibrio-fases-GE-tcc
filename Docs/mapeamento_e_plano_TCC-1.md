...
**Padrão de UI escolhido — selo + ícone de detalhe (híbrido, decidido em 2026-09-12):** nem nota de rodapé fixa (compete visualmente com o gráfico, que é o elemento principal — prioridade já registrada na seção 1.4 para projeção em sala de aula) nem só um ícone flutuante (a origem do parâmetro não é detalhe opcional; se só aparecer ao clicar, o usuário pode nunca ver e interpretar uma regressão como se fosse dado validado). A solução:

- **Selo pequeno, colorido, sempre visível**, perto do parâmetro ou da legenda do gráfico — texto curto: "Fornecido", "Banco de dados" ou "Calculado", cada um com cor própria. Garante que a informação essencial nunca passa despercebida, sem competir com o gráfico.
- **Ícone de informação (ⓘ) grudado no selo**, que ao passar o mouse ou clicar abre o texto mais rico: o que foi assumido, de onde veio o dado, quantos pontos entraram na regressão. Guarda o detalhe sem poluir a tela.
- **Estado dinâmico**: pela "regra de ouro" da seção 2.2 (tudo atualiza em tempo real com o parâmetro), o selo troca de estado ao vivo — se o usuário digitar um valor manual depois de ver um calculado, o selo muda de "Calculado" para "Fornecido" na mesma hora, sem recarregar nada.

Padrão inspirado no que dashboards financeiros e ferramentas de BI usam para sinalizar "estimado" vs. "medido" — familiar, não exige aprendizado novo de quem usa. Ainda não implementado — pendente da integração (item 1 de "Próximos passos" no `CLAUDE.md`) e da própria regressão (item acima).
